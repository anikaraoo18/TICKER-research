"""ABG comparable-company P/E case; inputs are intentionally editable below."""
...
TARGET = {
    ...
    "price": Decimal("243.03"),
    "diluted_eps": Decimal("21.50"),
}
PEERS = [
    {
        "ticker": "AN",
        "name": "AutoNation",
        "price": Decimal("169.84"),
        "diluted_eps": Decimal("16.92"),
    },
    {
        "ticker": "GPI",
        "name": "Group 1",
        "price": Decimal("421.48"),
        "diluted_eps": Decimal("36.81"),
    },
]
...
def main():
    ...
    if len(valid_peers) == 1:
        ...
        print(f"- Reference estimate: {middle_pe:.6f}x; implied ABG price {money(implied_price)}")
    else:
        print(f"- Minimum: {low_pe:.6f}x")
        print(f"- Median: {middle_pe:.6f}x")
        print(f"- Maximum: {high_pe:.6f}x")
        print(f"- Implied ABG price range: {money(low_pe * target_eps)}–{money(high_pe * target_eps)}")
        print(f"- Median-implied ABG price: {money(middle_pe * target_eps)}")
    ...
    for removed_peer in valid_peers:
        ...
        print(
            f"- Remove {removed_peer['ticker']}: remaining median-implied ABG price "
